...
公共日志库位于 `cpp/Log/`，所有服务、客户端和其他 CMake 子项目都可以链接
`lidar_log`（也提供 `Log` 目标别名）并包含 `lidar_log/Logger.hpp`。日志默认同时写
终端和 `logs/lidar.log`；服务端和 GUI 会在入口分别配置为
`logs/lidar_sim_server.log`、`logs/lidar_gui.log`。单个文件上限为 100 MiB，最多保留
10 个文件（当前文件加 `.1` 到 `.9` 的备份）。
...
int main() {
    lidar_log::Logger::initialize({
        .file_path = "logs/my_service.log",
        .minimum_level = lidar_log::Level::info,
        .console = true,
        .file = true,
    });
    LIDAR_LOG_INFO("listening on port ", 19850);
    LIDAR_LOG_ERROR("connection failed: ", "timeout");
}
```

`LIDAR_LOG_TRACE`、`LIDAR_LOG_DEBUG`、`LIDAR_LOG_INFO`、`LIDAR_LOG_WARN`、
`LIDAR_LOG_ERROR` 和 `LIDAR_LOG_FATAL` 宏支持多个流式参数，日志接口本身线程安全。

当前测试：

- `lidar_log_test`：日志级别、文件输出和大小轮转保留数量；
...
| 路径 | 作用 |
|---|---|
| `cpp/Log/` | 进程共享日志库：终端输出、100 MiB 大小轮转和最多 10 个文件保留 |
