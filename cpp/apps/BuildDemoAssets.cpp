/**
 * @file BuildDemoAssets.cpp
 * @brief 演示资产生成 CLI：端到端运行流水线并写出可视化 Dashboard HTML。
 *
 * 该工具用于在构建阶段一次性生成演示产物：
 * - 读取 JSON 流水线配置（默认 configs/DefaultPipeline.json）；
 ...
 *
 * 支持以下命令行参数（顺序无关）：
 *   --config <path>        流水线 JSON 配置文件，默认 configs/DefaultPipeline.json
...
int main(int argc, char** argv) {
    try {
        // === 默认参数：未显式指定时使用的兜底值 ===
        std::filesystem::path config_path = "configs/DefaultPipeline.json"; ///< 流水线配置文件路径
