...
#include "lidar_client/TcpClient.hpp"
...
int main(int argc, char* argv[]) {
    ...
    lidar_client::DeviceStatusModel device_status;
    lidar_client::ScanCycleMonitor scan_monitor;

    processor.set_step_complete_callback([&](const lidar_client::StepResult& result) {
        auto scan_summary = scan_monitor.summary_for_timestamp(result.timestamp);
        std::cerr << "\n========================================\n";
        std::cerr << "Step " << step_counter << " | " << result.timestamp << "\n";
        std::cerr << "  Rays: " << result.raw_count << "\n";
        std::cerr << "  Ground: " << result.ground_measurements.size() << "\n";
        if (device_status.snapshot().valid) {
            std::cerr << "  Device: " << device_status.brief() << "\n";
        }
        if (!scan_summary.scan_cycle_id.empty()) {
            std::cerr << "  Scan cycle: " << scan_summary.received_rays
                      << "/" << scan_summary.expected_rays
                      << " rays, duplicates=" << scan_summary.duplicate_rays
                      << ", missing=" << scan_summary.missing_ray_indices.size()
                      << ", complete=" << (scan_summary.complete ? "yes" : "no")
                      << "\n";
        }
        ...
        step_summary.expected_raw_count = scan_summary.expected_rays;
        step_summary.missing_raw_count = static_cast<int>(scan_summary.missing_ray_indices.size());
        step_summary.duplicate_raw_count = scan_summary.duplicate_rays;
        ...
        // 写出该步的 JSON 结果
        lidar_core::Json step_json = lidar_core::Json::Object{
            {"timestamp", result.timestamp},
            {"raw_count", result.raw_count},
            {"device_status", device_status.to_json()},
            {"scan_cycle", scan_monitor.summary_to_json(scan_summary)},
        };

        // 处理后的射线（序列化为 L2 JSON）
        lidar_core::Json::Array processed_arr;
        ...
        // 地面观测
        lidar_core::Json::Array ground_arr;
        ...
        // 热点
        lidar_core::Json::Array hotspot_arr;
        for (const auto& hs : result.hotspots) {
    ...
    while (!g_should_stop && client.is_connected()) {
        ...
        try {
            ...
            device_status.update_from_frame(frame);
            scan_monitor.observe_frame(frame);
            ...
            if (frame.type == lidar_protocol::FrameType::status &&
                ...
                std::cerr << "[client] " << device_status.brief() << "\n";
            }
    ...
    if (processor.total_steps_completed() > 0) {
        ...
        json_report["device_status"] = device_status.to_json();
