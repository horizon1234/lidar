...
客户端会对当前仿真器的非麻痹型死时间响应和 afterpulse 泄漏做逆校正，并用逐距离门
质量位阻断低 overlap、饱和、低 SNR 和无效近场进入反演或 PM；校正系数仍属于仿真假设，
不能替代目标设备序列号对应的接收机标定。
未同时加载实测接收机标定和站点 PM 标定时，PM2.5、PM10 和热点数组保持为空；服务端
`lidar_product` 中的
快速摘要不会被当成定量 PM。详细流程和标定 JSON 合同见
[客户端算法初学者指南与当前实现](algorithm_processing_chain.md)。
